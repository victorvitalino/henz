...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		if (opt('allDaySlot')) {
			...
			s =
				...
				"<tr>" +
				"<th class='" + headerClass + " fc-agenda-axis allday'>" +
				(
					opt('allDayHTML') ||
		...
		while (slotTime < maxTime) {
			...
			s +=
				...
				"<th class='time fc-agenda-axis " + headerClass + "'><div class='time-wrapper'>" +
				((!slotNormal || !minutes) ?
		...
		allDayCell = $('.fc-agenda-allday').find('th').slice(1,7);
	}
	...
	function buildDayTableHeadHTML() {
		...
		if (opt('weekNumbers')) {
			...
			html += "<th class='calendar-week-header fc-agenda-axis " + headerClass + "'>&nbsp;</th>";
		}
		...
		for (col=0; col<colCnt; col++) {
			...
			html +=
				"<th class='calendar-week-header fc-" + dayIDs[date.day()] + " fc-col" + col + ' ' + headerClass + " "+ activeHeaderClass+"'><div class='calendar-header-wrapper'><span class='day'>" +
				htmlEscape(moment(date).format("dddd")) + "</span>" +
				"<span class='date'>" +
				htmlEscape(moment(date).format("Do")) + "</span>" +
				"</div></th>";
			activeHeaderClass="";
		}
	...
	function setWidth(width) {
		...
		setOuterWidth(allDayCell, colWidth);
		recheckGridWith();
		//$('.fc-agenda-allday tbody th:nth-child(2)').width(colWidth+1);
	}
	
	var isInit = true;
	function recheckGridWith(){
		if(isInit !=true){
	 	 $('.fc-agenda-allday tbody th:nth-child(2)').width(colWidth);
		}
		else{
	  		$('.fc-agenda-allday tbody th:nth-child(2)').width(colWidth+1);
		}
		isInit =false;
		// console.log("col wid :"+colWidth);
		// console.log("col wid new :"+ (colWidth+1));
	}
...
function AgendaEventRenderer() {
	...
	function slotSegHtml(event, seg) {
		...
		if (url) {
			html += "a href='" + htmlEscape(event.url) + "' rel='popover' ";
		}else{
			html += "div rel='popover'";
		}
...
function DayEventRenderer() {
	...
	function buildHTMLForSegment(segment) {
		...
		if (url) {
			html += "<a href='" + htmlEscape(url) + "' rel='popover'";
		}else{
			html += "<div rel='popover'";
		}
		html +=
